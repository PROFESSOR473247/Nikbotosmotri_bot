...
import pytz
from database import load_tasks, save_tasks, load_templates, get_user_accessible_groups, is_authorized, get_user_role
...
class TaskManager:
    ...
    async def show_templates_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_role in ["гость", "водитель"]:
            await update.message.reply_text("❌ Недостаточно прав для управления шаблонами")
            return
