...
def debug_delete_template(template_id):
    ...
    logger.info(f"🔍 ДЕТАЛЬНАЯ ОТЛАДКА УДАЛЕНИЯ ШАБЛОНА {template_id}")
    ...
    try:
        ...
        logger.info("2. Проверка существования шаблона в БД...")
        cursor = conn.cursor()
        cursor.execute('SELECT id, name FROM templates WHERE id = %s', (template_id,))
        template_row = cursor.fetchone()
        
        if not template_row:
            logger.error(f"❌ Шаблон {template_id} не найден в базе данных")
            cursor.close()
            conn.close()
            return False
        
        template_name = template_row[1]
        logger.info(f"✅ Шаблон найден: ID={template_id}, Name={template_name}")
        
        # 3. Пробуем удалить
        logger.info("3. Попытка удаления шаблона...")
        cursor.execute('DELETE FROM templates WHERE id = %s', (template_id,))
        deleted_count = cursor.rowcount
        
        if deleted_count > 0:
            logger.info(f"✅ Удалено записей: {deleted_count}")
            conn.commit()
            result = True
        else:
            logger.error("❌ Не удалось удалить запись (rowcount = 0)")
            conn.rollback()
            result = False
...
def debug_list_all_templates():
    ...
    logger.info("📋 ВСЕ ШАБЛОНЫ В БАЗЕ ДАННЫХ:")
    ...
    try:
        ...
        if not templates:
            logger.info("📭 В базе данных нет шаблонов")
        else:
            for i, (template_id, name, group_name) in enumerate(templates, 1):
                logger.info(f"{i}. ID: {template_id}, Name: {name}, Group: {group_name}")
